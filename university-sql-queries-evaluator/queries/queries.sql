...
SELECT a.nombre, ce.anyo_inicio, ce.anyo_fin FROM persona p INNER JOIN alumno_se_matricula_asignatura am ON p.id = am.id_alumno INNER JOIN asignatura a ON am.id_asignatura = a.id INNER JOIN curso_escolar ce ON am.id_curso_escolar = ce.id WHERE p.nif = '26902806M';
...
SELECT DISTINCT d.nombre FROM departamento d INNER JOIN profesor pr ON d.id = pr.id_departamento INNER JOIN asignatura a ON pr.id_profesor = a.id_profesor INNER JOIN grado g ON a.id_grado = g.id WHERE g.nombre = 'Grado en Ingeniería Informática (Plan 2015)';
...
SELECT DISTINCT p.nombre, p.apellido1, p.apellido2 FROM persona p INNER JOIN alumno_se_matricula_asignatura am ON p.id = am.id_alumno INNER JOIN curso_escolar ce ON am.id_curso_escolar = ce.id WHERE ce.anyo_inicio = 2018 AND ce.anyo_fin = 2019;
...
SELECT p.apellido1, p.apellido2, p.nombre FROM persona p INNER JOIN profesor pr ON p.id = pr.id_profesor LEFT JOIN asignatura a ON pr.id_profesor = a.id_profesor WHERE a.id_profesor IS NULL;

-- 14. Retorna un llistat amb les assignatures que no tenen un professor/a assignat. (id, nombre)
SELECT a.id, a.nombre FROM asignatura a LEFT JOIN profesor pr ON a.id_profesor = pr.id_profesor WHERE pr.id_profesor IS NULL;
...
SELECT d.nombre AS departamento, COUNT(pr.id_profesor) AS total FROM departamento d INNER JOIN profesor pr ON d.id = pr.id_departamento GROUP BY d.nombre ORDER BY total DESC;
...
SELECT d.nombre AS departamento, COUNT(pr.id_profesor) AS total FROM departamento d LEFT JOIN profesor pr ON d.id = pr.id_departamento GROUP BY d.nombre;

-- 20. Retorna un llistat amb el nom de tots els graus existents en la base de dades i el nombre d'assignatures que té cadascun. Tingues en compte que poden existir graus que no tenen assignatures associades. Aquests graus també han d'aparèixer en el llistat. El resultat haurà d'estar ordenat de major a menor pel nombre d'assignatures. (grau, total)
SELECT g.nombre AS grado, COUNT(a.id) AS total FROM grado g LEFT JOIN asignatura a ON g.id = a.id_grado GROUP BY g.nombre ORDER BY total DESC;
...
SELECT g.nombre AS grau, count(a.id) AS total FROM grado g LEFT JOIN asignatura a ON g.id = a.id_grado GROUP by g.nombre HAVING total > 40;
...
SELECT g.nombre AS grau , a.tipo, sum(a.creditos) AS total_creditos FROM grado g JOIN asignatura a ON g.id = a.id_grado GROUP by g.nombre, a.tipo;
...
SELECT c.anyo_inicio, count(distinct m.id_alumno) AS total from curso_escolar c JOIN alumno_se_matricula_asignatura m ON c.id = m.id_curso_escolar GROUP by c.anyo_inicio;
...
SELECT p.id, p.nombre, p.apellido1, p.apellido2, count( a.id) AS total FROM persona p JOIN profesor pr ON p.id = pr.id_profesor LEFT JOIN asignatura a ON pr.id_profesor =	a.id_profesor GROUP BY p.id Order by total DESC;

-- 25. Retorna totes les dades de l'alumne/a més jove. (*)
SELECT * FROM persona p WHERE tipo = 'alumno' AND fecha_nacimiento = (SELECT MAX(fecha_nacimiento)FROM persona WHERE tipo='alumno');
...
SELECT DISTINCT p.apellido1, p.apellido2, p.nombre FROM persona p JOIN profesor pr ON p.id = pr.id_profesor LEFT JOIN asignatura a ON pr.id_profesor = a.id_profesor WHERE pr.id_departamento IS NOT NULL AND a.id IS NULL;